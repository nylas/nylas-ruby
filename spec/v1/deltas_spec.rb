...
describe Nylas::API do
  ...
  describe 'Delta sync API wrapper' do
    ...
    it 'returns an external Enumerator when no block is given' do
      result = inbox.deltas(nth_cursor, [File], false, [Folder])
    ...
    it 'should continuously query the delta sync API' do
      count = 0
      inbox.deltas(0, [File], false, [Folder]) do |event, object|
        expect(object.cursor).to_not be_nil
        if event == 'create' or event == 'modify'
          expect(object).to be_a Message
        elsif event == 'delete'
          expect(object).to be_a Event
        end
        count += 1
      end
  ...
  describe 'Delta sync streaming API wrapper' do
    ...
    it 'should continuously query the delta sync API' do
      count = 0
      EM.run do
        inbox.delta_stream(0, [Folder], 0, false, [File]) do |event, object|
