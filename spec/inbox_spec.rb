...
describe 'Inbox' do
  ...
  describe "#url_for_authentication" do
    ...
    it "should return the OAuth authorize endpoint with the provided redirect_uri" do
      ...
      expect(url).to eq("https://www.nylas.com/oauth/authorize?client_id=#{@app_id}&trial=false&response_type=code&scope=email&login_hint=&redirect_uri=http://redirect.uri")
    end

    it "should include the login_hint if one is provided" do
      url = @inbox.url_for_authentication('http://redirect.uri', 'ben@nylas.com')
      expect(url).to eq("https://www.nylas.com/oauth/authorize?client_id=#{@app_id}&trial=false&response_type=code&scope=email&login_hint=ben@nylas.com&redirect_uri=http://redirect.uri")
    end
    ...
    it "should use trial=true if the trial flag is passed" do
      ...
      expect(url).to eq("https://www.nylas.com/oauth/authorize?client_id=#{@app_id}&trial=true&response_type=code&scope=email&login_hint=ben@nylas.com&redirect_uri=http://redirect.uri")
    end
  end
